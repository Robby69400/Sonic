...
const int16_t desired_rssi = (-95 + 160) * 2; // -95dBm: AGC headroom for weak signals
...
void AM_fix_10ms(const unsigned vfo)
{
    ...
    if (diff_dB > 0) {  // decrease gain
        ...
        if (gain_table_index[vfo] != index)
        {
            ...
            hold_counter[vfo] = 80;       // 800ms hold - prevents AGC hunting on AM speech
        }
    }

    if (diff_dB >= -10)                   // 10dB hysteresis (wider dead-band = less hunting on AM)
        hold_counter[vfo] = 80;           // 800ms hold
