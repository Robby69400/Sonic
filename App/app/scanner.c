...
void SCANNER_Stop(void)
{
    if(SCANNER_IsScanning()) {
        // Clear any pending BK4819 interrupts BEFORE stopping scan
        // to prevent infinite loop in RADIO_SetupRegisters() when
        // chip is left in a state where REG_0C bit0 never clears
        // (happens after BK4819_Disable() is called on CSS FOUND)
        BK4819_WriteRegister(BK4819_REG_3F, 0);  // disable interrupt mask
        BK4819_WriteRegister(BK4819_REG_02, 0);  // clear pending interrupts
